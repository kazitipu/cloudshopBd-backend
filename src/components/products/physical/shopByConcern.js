...
export class Tags extends Component {
  ...
  handleSubmit = async () => {
    ...
    if (this.state.type === "upload") {
      ...
      await this.props.uploadTagRedux(tagObj);
  ...
  render() {
    ...
    const { tags, currentAdmin } = this.props;

    let renderableTags = tags;
    if (this.state.searchFor) {
      renderableTags = tags.filter((tag) =>
        tag.name.toLowerCase().includes(this.state.searchFor.toLowerCase())
      );
    }
    renderableTags = renderableTags.sort(
      (a, b) => a.name.charCodeAt(0) - b.name.charCodeAt(0)
    );

    return (
      <Fragment>
        <Breadcrumb title={"Shop By Concern"} parent="Products" />
                              ...
                              <div>
                                ...
                                >
                                  <input
                                    ...
                                    onChange={(e) => this.selectRow(e, tag.id)}
                                  />
                                </span>
                              </div>
                            </th>
              ...
              <div className="modal-body">
                <div style={{ padding: "10px 15px" }}>
                  <div>
                    ...
                    {this.state.checkedValues.length} tags?
                  </div>
                </div>
                <table className="table table-bordered table-striped table-hover">
                  <thead>
                ...
                <button
                  ...
                  onClick={async () => {
                    this.state.checkedValues.map(async (id) => {
                      await this.props.deleteTagRedux(id);
                    });
                    this.setState({
                      selectAll: false,
                      checkedValues: [],
